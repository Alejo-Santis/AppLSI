...
class StoreEmployeeRequest extends FormRequest
{
        ...
        return [
            'first_name' => [
                ...
                function ($attribute, $value, $fail) {
                    if (trim($value) === '') {
                        $fail('El nombre no puede estar vacío o contener solo espacios.');
                    }
            ...
            'last_name' => [
                ...
                function ($attribute, $value, $fail) {
                    if (trim($value) === '') {
                        $fail('El apellido no puede estar vacío o contener solo espacios.');
                    }
                    if (preg_match('/^(\d)\1+$/', trim($value))) {
            ...
            'hire_date' => [
                ...
                'after:1950-01-01',
                function ($attribute, $value, $fail) {
                    $hireDate = \Carbon\Carbon::parse($value);
                    $today = \Carbon\Carbon::today();
                    
                    if ($hireDate->isFuture()) {
                        $fail('La fecha de contratación no puede ser futura.');
                    }
                    
                    if ($hireDate->diffInYears($today) > 70) {
                        $fail('La fecha de contratación no puede ser mayor a 70 años atrás.');
                    }
                },
            ],
            'salary' => [
                ...
                function ($attribute, $value, $fail) {
                    if ($value < 500000) {
                        $fail('El salario parece demasiado bajo. Verifica el valor (mínimo recomendado: $500,000).');
                    }
                    if ($value > 50000000) {
                        $fail('El salario parece inusualmente alto. Verifica el valor ingresado.');
                    }
                },
            ],
            'status' => [
            ...
            'birth_date' => [
                ...
                function ($attribute, $value, $fail) {
                    if ($value) {
                        ...
                        if ($age > 100) {
                            $fail('La fecha de nacimiento parece incorrecta. Verifica el valor.');
                        }
                        
                        // Validar que la fecha de contratación sea después de cumplir 16 años
                        $hireDate = $this->input('hire_date');
                        if ($hireDate) {
                            $hireDateParsed = \Carbon\Carbon::parse($hireDate);
                            $minHireAge = $birthDate->copy()->addYears(16);
                            
                            if ($hireDateParsed->lt($minHireAge)) {
                                $fail('La fecha de contratación debe ser al menos cuando el empleado tenga 16 años.');
                            }
                        }
                    }
                },
            ...
            'address' => [
                ...
                function ($attribute, $value, $fail) {
                    if ($value && trim($value) === '') {
                        $fail('La dirección no puede estar vacía o contener solo espacios.');
                    }
                },
            ],
            'photo' => [
    ...
    public function messages(): array
    {
        return [
            ...
            'hire_date.after' => 'La fecha de contratación debe ser posterior a 1950.',
            ...
            'birth_date.after' => 'La fecha de nacimiento debe ser posterior a 1920.',
